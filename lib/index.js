...
function isArtifact(name) {
	if (!ARTIFACT_PREFIXES.some((p) => name.startsWith(p))) return false;
	return ARTIFACT_SUFFIXES.some((s) => name.endsWith(s));
}
/** Validate retention options; returns an error string when misconfigured. */
function validateRetention(options) {
	const { retentionDays, retentionMaxFiles } = options;
	if (retentionDays !== void 0 && (!Number.isFinite(retentionDays) || retentionDays <= 0)) return `retentionDays must be a positive number (got ${String(retentionDays)})`;
	if (retentionMaxFiles !== void 0 && (!Number.isFinite(retentionMaxFiles) || retentionMaxFiles < 1 || !Number.isInteger(retentionMaxFiles))) return `retentionMaxFiles must be an integer >= 1 (got ${String(retentionMaxFiles)})`;
}
/**
...
*/
async function applyRetention(dir, options) {
	const empty = {
		deleted: [],
		considered: 0
	};
	if (options.retentionDays === void 0 && options.retentionMaxFiles === void 0) return empty;
	if (validateRetention(options) !== void 0) return empty;
	try {
	...
	for (const name of names) {
		...
		try {
			...
			if (info.isFile()) candidates.push({
				path,
	...
	const cutoff = options.retentionDays !== void 0 ? Date.now() - options.retentionDays * 864e5 : void 0;
	const toDelete = /* @__PURE__ */ new Set();
	if (cutoff !== void 0) {
		for (const file of candidates) if (file.mtimeMs < cutoff) toDelete.add(file.path);
	}
	if (options.retentionMaxFiles !== void 0) {
		const survivors = candidates.filter((f) => !toDelete.has(f.path)).sort((a, b) => a.mtimeMs - b.mtimeMs);
		const excess = survivors.length - options.retentionMaxFiles;
		for (let i = 0; i < excess; i += 1) {
			const victim = survivors[i];
			if (victim !== void 0) toDelete.add(victim.path);
		}
	}
	const deleted = [];
	for (const victim of toDelete) try {
		await unlink(victim);
		const record = candidates.find((c) => c.path === victim);
		if (record !== void 0) deleted.push(record);
	} catch {}
	return {
		deleted,
...
function describeRetention(result) {
...
*/
function applyContract(contract, requested) {
	const overlay = {};
	if (contract.requireMask === true) overlay.mask = true;
	if (contract.requireMaskMode !== void 0) overlay.maskMode = contract.requireMaskMode;
	return {
		...requested,
		...overlay
	};
...
*/
function checkFormats(contract, requested) {
	const allowed = contract.allowedFormats;
	if (allowed === void 0 || requested.length === 0) return void 0;
	const bad = requested.filter((f) => !allowed.includes(f));
	if (bad.length === 0) return void 0;
...
*/
function checkPinnedDir(contract, outPath) {
	if (contract.pinnedDir === void 0 || outPath === void 0) return void 0;
	const normalizedPin = contract.pinnedDir.replace(/\/+$/, "");
	const normalizedOut = outPath.replace(/\/+$/, "");
	if (normalizedOut === normalizedPin || normalizedOut.startsWith(`${normalizedPin}/`)) return void 0;
...
function describeContract(contract) {
	...
	if (contract.requireMaskMode !== void 0) parts.push(`maskMode=${contract.requireMaskMode}`);
	if (contract.allowedFormats !== void 0) parts.push(`formats: ${contract.allowedFormats.join("|")}`);
	if (contract.pinnedDir !== void 0) parts.push(`dir: ${contract.pinnedDir}`);
...
function parseExportFormat(token) {
...
*/
function applyRules(text, rules, mode) {
	let out = text;
	for (const rule of rules) if (mode === "hash") out = out.replace(rule.pattern, (...groups) => {
		const full = groups[0] ?? "";
		const prefix = typeof groups[1] === "string" ? groups[1] : "";
		return prefix + hashMarker(full.slice(prefix.length));
	});
	else out = out.replace(rule.pattern, rule.replacement);
	return out;
}
/** Deterministic short digest marker: same secret → same marker, forever. */
function hashMarker(secret) {
...
* @param rules - Compiled rules.
*/
function maskMessage(message, rules, mode) {
	for (const block of message.content) if (block.type === "text" || block.type === "reasoning") block.text = applyRules(block.text, rules, mode);
	else if (block.type === "tool-call") block.arguments = applyRules(block.arguments, rules, mode);
	else if ("content" in block) {
		for (const inner of block.content) if (inner.type === "text") inner.text = applyRules(inner.text, rules, mode);
	}
}
/**
...
function describeArtifact(path, content) {
	return {
		path,
...
function describeBinaryArtifact(path, data) {
	return {
		path,
		bytes: data.length,
		sha256: sha256Binary(data)
	};
}
...
function buildManifest(input) {
	return {
		...
		scope: {
			...
			...input.scope.since !== void 0 ? { since: input.scope.since } : {},
...
function verifyManifest(manifest, artifacts) {
	const mismatches = [];
	let checked = 0;
...
const USAGE = "Usage: /transcript [path] [--id <sessionId>] [--out <path>] [--json] [--md] [--html] [--full] [--last <duration>] [--errors-only] [--mask] [--mask-hash] [--manifest]";
...
function transcriptBasePath(baseDir, sessionIdRaw) {
...
const GENERATOR = "dsh-session-export v1.8.0";
...
async function executeTranscript(ctx, invocation, config) {
	...
	const maskMode = !maskOn ? "off" : args.maskHash ? "hash" : config?.maskMode ?? "mask";
	const renderedEntries = maskOn ? maskEntries(entries, {
		extraPatterns: config?.maskPatterns,
		mode: maskMode === "hash" ? "hash" : "mask"
	}) : entries;
	const input = {
		header: log.session,
	...
	const outputs = [];
	const defaultPath = args.outPath !== void 0 ? void 0 : transcriptBasePath(baseDir, String(sessionIdRaw));
	if (args.md) {
	...
	let manifestPath;
	if (manifestOn) manifestPath = defaultPath !== void 0 ? `${defaultPath}.manifest.json` : `${(args.outPath ?? "transcript").replace(/\.(md|html|json)$/i, "")}.manifest.json`;
	...
	const manifestNote = manifestPath !== void 0 ? ` (+ ${manifestPath})` : "";
	const maskNote = maskMode !== "off" ? `, ${maskMode}-redacted` : "";
	let retentionNote = "";
	if (config?.retentionDays !== void 0 || config?.retentionMaxFiles !== void 0) {
		const dir = outputs[0] !== void 0 ? outputs[0].path.split("/").slice(0, -1).join("/") : void 0;
		if (dir !== void 0 && dir.length > 0) {
			const line = describeRetention(await applyRetention(dir, {
...
function exportError(sessionId, format, message) {
	return {
		sessionId,
		format,
		written: [],
...
function renderResultText(result) {
	...
	return `Exported ${result.messages} messages (${result.toolCalls} tool calls, ${result.tokens} tokens${maskNote}) → ${result.written.join(", ")}${manifestNote}`;
}
...
*/
function createExportTool(config, engine) {
	return defineTool({
		...
		parameters: {
			session_id: {
				type: "string",
				required: true,
				description: "Id of the session to export (e.g. a recall hit sessionId)."
			},
			format: {
				type: "string",
				description: "Output format: 'html' (default), 'md', or 'json'."
			},
			mask: {
				type: "boolean",
				description: "Redact likely secrets (API keys, bearer tokens, private keys, emails) from the report."
			},
			manifest: {
				type: "boolean",
				description: "Also write a .manifest.json sidecar with the artifact sha256."
			},
			last: {
				type: "string",
				description: "Limit to entries from the recent past, e.g. 30m / 12h / 7d."
			}
		},
		output: {
		...
		async execute(args) {
			const format = args.format === "md" || args.format === "json" ? args.format : "html";
			...
			let entries = buildEntries(log.events);
			if (since !== void 0) entries = entries.filter((entry) => entry.time >= since);
			const filterNote = since !== void 0 ? "Filtered view: entries within last" : void 0;
			const totals = buildTotals(entries);
			const stats = computeStats(entries, config?.pricing);
			const maskOn = args.mask === true || config?.mask === true;
			const maskMode = !maskOn ? "off" : config?.maskMode ?? "mask";
			const renderedEntries = maskOn ? maskEntries(entries, {
				extraPatterns: config?.maskPatterns,
				mode: maskMode === "hash" ? "hash" : "mask"
			}) : entries;
			...
			const base = transcriptBasePath(config?.defaultDir ?? log.session.cwd ?? process.cwd(), sessionIdRaw);
			...
			const manifestPath = args.manifest === true || config?.manifest === true ? `${base}.manifest.json` : void 0;
			try {
				await atomicWriteFile(path, content);
				if (manifestPath !== void 0) await atomicWriteFile(manifestPath, renderManifest(buildSessionManifest({
					createdAt: input.generatedAt,
					session: log.session,
					scope: {
						entries: entries.length,
...
function buildArchiveFromLog(log) {
...
async function buildArchiveZip(ctx, header) {
	return buildArchiveFromLog(await ctx.sessionQuery.readSession(header.id));
}
...
const STATS_USAGE = "Usage: /stats [--id <sessionId>] [--json] [--out <path>]";
/** Parse raw command input; returns args or a usage-error string. */
function parseStatsArgs(rawInput) {
	...
	while (i < tokens.length) {
		...
		if (token === "--out") {
			const value = tokens[i + 1];
			if (value === void 0 || value.startsWith("--")) return `--out requires a path value.\n${STATS_USAGE}`;
			if (args.outPath !== void 0) return `--out may be given only once.\n${STATS_USAGE}`;
			args.outPath = value;
			i += 2;
			continue;
		}
...
async function executeStats(ctx, invocation, config) {
	...
	if (parsed.json === true) {
		const payload = JSON.stringify({
			generator: "dsh-session-export v1.8.0",
			stats
		}, null, 2);
		if (parsed.outPath !== void 0) {
			...
			return {
				kind: "success",
				text: `Stats JSON written → ${parsed.outPath}`
			};
		}
		return {
			kind: "success",
	...
	if (parsed.outPath !== void 0) {
		...
		return {
			kind: "success",
			text: `Stats card written → ${parsed.outPath}`
		};
	}
	return {
		kind: "success",
		text: formatStatsCard(stats)
	};
}
//#endregion
//#region src/bundleCommand.ts
const BUNDLE_USAGE = [
...
function parseBundleArgs(rest) {
	...
	while (i < tokens.length) {
		...
		if (token === "--format") {
			const value = tokens[i + 1];
			if (value === void 0 || value.startsWith("--")) return `--format requires a format (md, html, or json).\n${BUNDLE_USAGE}`;
			switch (value) {
				case "md":
					args.md = true;
					break;
		...
		if (token === "--no-transcript") {
			args.noTranscript = true;
			i += 1;
			continue;
		}
		if (token === "--no-archive") {
...
async function executeBundle(ctx, invocation, config) {
	...
	const args = parsed;
	if (args.noTranscript && args.noArchive) return {
		kind: "error",
		text: "At least one of transcript or archive is required.\n" + BUNDLE_USAGE
	};
	const sessionIdRaw = args.sessionId ?? invocation.agent.session.id;
	const sessionId = SessionId(sessionIdRaw);
	let log;
	try {
		...
		return {
			kind: "error",
			text: `Could not read session ${sessionIdRaw}: ${error instanceof Error ? error.message : String(error)}`
		};
	}
	let lineage;
	try {
	...
	if (!args.noTranscript) {
		if (args.md) {
			bundleEntries.push({
				name: "transcript.md",
				data: new TextEncoder().encode(renderMarkdown(input, config))
			});
			formatNames.push("md");
		}
		if (args.html) {
			bundleEntries.push({
				...
				data: new TextEncoder().encode(renderHtml(input, config))
			});
			formatNames.push("html");
		}
		if (args.json) {
			bundleEntries.push({
				name: "transcript.json",
				data: new TextEncoder().encode(renderJson(input))
			});
	...
	if (args.manifest || config?.manifest === true) {
		...
		bundleEntries.push({
			name: "manifest.json",
			data: new TextEncoder().encode(renderManifest(manifest))
		});
	}
	const bundleZip = buildZip(bundleEntries);
	const baseDir = config?.defaultDir ?? log.session.cwd ?? process.cwd();
	const outPath = args.outPath ?? `${baseDir}/dsh-transcripts/bundle-${id8(sessionIdRaw)}-${timestampSlug(Date.now())}.zip`;
	try {
		...
		return {
			kind: "error",
			text: `Failed to write bundle: ${error instanceof Error ? error.message : String(error)}`
		};
	}
	const parts = [];
...
function parseDiffArgs(rest) {
	...
	while (i < tokens.length) {
		...
		if (token === "--id") {
			const v = tokens[i + 1];
			if (v === void 0 || v.startsWith("--")) return `--id requires a session id value.\n${DIFF_USAGE}`;
			result.sessionIdA = v;
			i += 2;
			continue;
		}
		if (token === "--other") {
			const v = tokens[i + 1];
			if (v === void 0 || v.startsWith("--")) return `--other requires a session id value.\n${DIFF_USAGE}`;
			result.sessionIdB = v;
			i += 2;
			continue;
		}
...
function entryFingerprint(entry) {
	...
	return `${entry.kind}:${entry.error !== void 0 ? "ERR" : ""}:${text}`;
}
/** Compute the diff of two entry arrays. */
function diffSessions(entriesA, entriesB) {
	...
	while (commonSuffixLen < entriesA.length - commonPrefixLen && commonSuffixLen < entriesB.length - commonPrefixLen && entryFingerprint(entriesA[entriesA.length - 1 - commonSuffixLen]) === entryFingerprint(entriesB[entriesB.length - 1 - commonSuffixLen])) commonSuffixLen += 1;
...
*/
function classifyChanges(midA, midB, prefixLen) {
	const blocks = [];
	let ia = 0;
	let ib = 0;
	while (ia < midA.length || ib < midB.length) {
		...
		if (runA > 0 && runB === 0) {
			blocks.push({
				kind: "removed",
				rangeA: [prefixLen + startA, prefixLen + startA + runA - 1],
				rangeB: [prefixLen + startB - 1, prefixLen + startB - 1],
				entriesA: midA.slice(startA, startA + runA),
				entriesB: []
			});
			ia += runA;
			...
			blocks.push({
				kind: "added",
				rangeA: [prefixLen + startA - 1, prefixLen + startA - 1],
				rangeB: [prefixLen + startB, prefixLen + startB + runB - 1],
				entriesA: [],
				entriesB: midB.slice(startB, startB + runB)
			});
			ib += runB;
		} else {
			blocks.push({
...
function renderTerminalDiff(result) {
	const { sessionA, sessionB, commonPrefixLen, tailA, tailB, totalA, totalB, totals } = result;
	const header = [
		"=== Session Diff ===",
		"",
		`A: ${sessionA.id}  (${new Date(sessionA.createdAt).toISOString()})${sessionA.cwd !== void 0 ? `  cwd: ${sessionA.cwd}` : ""}`,
		`B: ${sessionB.id}  (${new Date(sessionB.createdAt).toISOString()})${sessionB.cwd !== void 0 ? `  cwd: ${sessionB.cwd}` : ""}`,
		"",
...
async function executeDiff(ctx, invocation, config) {
	...
	try {
		...
		return {
			kind: "error",
			text: `Could not read sessions: ${error instanceof Error ? error.message : String(error)}`
		};
	}
	...
	computeStats(entriesA, config?.pricing);
	computeStats(entriesB, config?.pricing);
	const result = {
		sessionA: {
			id: logA.session.id,
			createdAt: logA.session.createdAt,
			cwd: logA.session.cwd
		},
		sessionB: {
			id: logB.session.id,
			createdAt: logB.session.createdAt,
			cwd: logB.session.cwd
		},
	...
	if (args.html) {
		...
		try {
			...
			return {
				kind: "success",
				text: `Diff report written → ${outPath}`
			};
		} catch (error) {
			return {
				kind: "error",
				text: `Failed to write diff report: ${error instanceof Error ? error.message : String(error)}`
			};
		}
	}
	return {
		kind: "success",
		text: renderTerminalDiff(result)
	};
}
/** Build a minimal single-file HTML diff report using the shared HTML report CSS conventions. */
function renderDiffHtml(result, config) {
	const tailA = result.tailA;
	const tailB = result.tailB;
	const commonLen = result.commonPrefixLen;
	const entryHtml = (entry, side) => {
		return `<div class="${entry.error !== void 0 ? "entry error" : "entry"} side-${side}"><span class="kind">${esc(entry.kind)}</span> <span class="time">${new Date(entry.time).toISOString()}</span><pre>${esc(extractText(entry))}</pre>${entry.error !== void 0 ? `<div class="error-msg">${esc(entry.error.code)}</div>` : ""}</div>`;
	};
	const tailAHtml = tailA.slice(0, 50).map((e) => entryHtml(e, "a")).join("\n");
	const tailBHtml = tailB.slice(0, 50).map((e) => entryHtml(e, "b")).join("\n");
	const kindBadge = (k) => k === "added" ? "<span class=\"badge added\">＋ added</span>" : k === "removed" ? "<span class=\"badge removed\">－ removed</span>" : "<span class=\"badge changed\">± changed</span>";
	const changesSection = result.changes.length > 0 ? `<h2>Changes (${result.changes.length} block${result.changes.length === 1 ? "" : "s"})</h2>\n` + result.changes.slice(0, 30).map((c) => `<div class="change-block">${kindBadge(c.kind)} <span class="range">A[${c.rangeA[0]}..${c.rangeA[1]}] ↔ B[${c.rangeB[0]}..${c.rangeB[1]}]</span>\n${c.entriesA.length > 0 ? `<div class="change-side">A:</div>${c.entriesA.slice(0, 10).map((e) => entryHtml(e, "a")).join("")}` : ""}${c.entriesB.length > 0 ? `<div class="change-side">B:</div>${c.entriesB.slice(0, 10).map((e) => entryHtml(e, "b")).join("")}` : ""}</div>`).join("\n") + (result.changes.length > 30 ? `<p class="more">… and ${result.changes.length - 30} more blocks</p>` : "") : "";
	return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8">
...
function resolvePreset(config) {
	...
	if (preset === void 0) throw new Error(`Unknown preset "${presetName}". Valid presets: ${Object.keys(PRESETS).join(", ")}`);
	return {
		...preset,
		...config
	};
...
function apply(ctx, config) {
	...
	if (config?.exposeTool === true) ctx.effect(function* () {
