...
function renderKpiGrid(input) {
	const stats = input.stats;
	const cards = [
		["Messages", String(input.totals.messages)],
		[
			"Tool calls",
			String(input.totals.toolCalls),
			stats && stats.failedToolCalls > 0 ? `${stats.failedToolCalls} failed` : void 0
		],
		["Tokens in", fmtTokens(input.totals.inputTokens)],
		["Tokens out", fmtTokens(input.totals.outputTokens)],
		["Duration", stats?.durationMs !== void 0 && stats.durationMs !== null ? formatDuration(stats.durationMs) : "—"],
		["Turns", stats !== void 0 ? String(stats.turns) : "—"]
	];
	const cost = stats?.cost;
	if (cost !== void 0) cards.push(["Cost ≈", `${cost.currency}${cost.total >= .01 ? cost.total.toFixed(2) : cost.total.toFixed(4)}`]);
	const spark = stats !== void 0 ? renderSparklineSvg(stats.perAssistantTokens) : "";
	return [
		"<section class=\"kpi-grid\">",
		...cards.map(([label, value, note]) => `<div class="kpi"><div class="kpi-label">${escapeHtml(label)}</div><div class="kpi-value">${escapeHtml(value)}${note !== void 0 ? ` <span class="kpi-note">${escapeHtml(note)}</span>` : ""}</div></div>`),
...
function renderSparklineSvg(series) {
	...
	return `<svg class="sparkline" viewBox="0 0 ${W} ${H}" preserveAspectRatio="none" role="img" aria-label="output tokens per assistant message">${series.map((value, index) => {
...
function renderTimeline(input) {
	...
	if (starts.length < 2 || durationMs === void 0 || durationMs === null) return "";
	const totalMs = durationMs;
	return `<section class="card"><h2>Turn timeline</h2>${starts.map((start, index) => {
...
function renderToolRanking(input) {
	const breakdown = input.stats?.toolBreakdown ?? [];
	if (breakdown.length === 0) return "";
	const max = breakdown[0]?.calls ?? 1;
	return `<section class="card"><h2>Tool calls</h2>${breakdown.slice(0, 8).map((tool) => {
...
function renderBlocks(blocks, options) {
	const parts = [];
	for (const block of blocks) if (block.type === "reasoning") parts.push(`<details class="reasoning"><summary>Reasoning</summary><pre>${escapeHtml(block.text.trim())}</pre></details>`);
	else if (block.type === "text") parts.push(`<div class="msg-text">${escapeHtml(block.text.trim())}</div>`);
	else if (block.type === "tool-call") {
		const parsed = parseToolArguments(block.arguments);
		const diff = renderToolDiff(block.name, parsed);
		if (diff) parts.push(`<details class="tool"><summary>🔧 ${escapeHtml(block.name)}</summary><pre class="diff">${escapeHtml(truncate(diff, options.resultCharLimit))}</pre></details>`);
		else parts.push(`<details class="tool"><summary>🔧 ${escapeHtml(block.name)}</summary><pre><code>${escapeHtml(truncate(block.arguments, options.argCharLimit))}</code></pre></details>`);
		...
		parts.push(`<details class="tool-result"${block.isError === true ? " open" : ""}><summary>🧾 result</summary><pre>${escapeHtml(truncate(inner, options.resultCharLimit))}</pre></details>`);
...
function renderEntry(entry, options) {
	...
	if (toolResult !== void 0) {
		...
		const errorLine = entry.error !== void 0 ? `<div class="error-banner">Tool failed: <code>${escapeHtml(entry.error.name)}</code> (<code>${escapeHtml(entry.error.code)}</code>)</div>` : "";
		const body = renderBlocks(toolResult.content, options).join("\n");
		return `<article class="entry tool-result${isError ? " error" : ""}"><header>🧾 Tool result <code>${escapeHtml(id8$1(String(toolResult.source.callId)))}</code>${isError ? " <span class=\"fail\">⚠ ERROR</span>" : ""} <time>${time}</time></header>${errorLine}${body}</article>`;
	}
	return `<article class="entry user"><header>👤 User <time>${time}</time></header>${renderBlocks(message.content, options).join("\n")}</article>`;
}
...
function renderHtml(input, options) {
	...
	const title = `DSH session ${id8$1(h.id)} — transcript`;
	return `<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8">
...
<div class="meta">${escapeHtml(h.cwd ?? "")} · created ${escapeHtml(h.createdAt ? fmtTime(h.createdAt) : "?")}${h.agentPreset !== void 0 ? ` · ${escapeHtml(h.agentPreset)}` : ""}</div>
