...
function renderBlocks(blocks, options) {
	...
	const parts = [];
	for (const block of blocks) if (block.type === "reasoning") parts.push(`<details class="reasoning"><summary>${escapeHtml(t("reasoning", lang))}</summary><pre>${escapeHtml(block.text.trim())}</pre></details>`);
		...
		else parts.push(`<details class="tool"><summary>🔧 ${escapeHtml(block.name)}</summary><pre>${renderCode(block.arguments, options.argCharLimit)}</pre></details>`);
		...
		parts.push(`<details class="tool-result"${block.isError === true ? " open" : ""}><summary>🧾 ${escapeHtml(t("result", lang))}</summary><pre>${renderCode(inner, options.resultCharLimit)}</pre></details>`);
...
function renderHtml(input, options) {
	...
	const entries = input.entries.map((entry) => {
		const isError = entry.error !== void 0 || (asToolResult(entry.message)?.content.some((block) => block.isError === true) ?? false);
		return renderEntry(entry, opts, isError ? ++errorCount : void 0);
	}).join("\n");
